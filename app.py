...
# =========================================================================
# Definimos el diseño de la página de Streamlit en modo ancho (wide)
...
# Entrada de texto para personalizar el reporte con la marca del cliente
nombre_negocio = st.sidebar.text_input("1. Nombre de tu marca o negocio:", placeholder="Ej. Mi Emprendimiento")

# El cargador de archivos se activa únicamente si el usuario ha digitado el nombre del negocio
archivo_cargado = None
if nombre_negocio:
    st.sidebar.markdown("---")
    archivo_cargado = st.sidebar.file_uploader(f"2. Sube el CSV de Meta Business para '{nombre_negocio}'", type=["csv"])

# Flags de control de estado del dataset
data_lista = False
df_fb = None

# =========================================================================
# 3. GUÍA VISUAL E ILUSTRADA DE EXTRACCIÓN (ONBOARDING DE UX REAL)
# =========================================================================
# Esta sección se muestra AL INICIO, únicamente si el CSV no ha sido cargado aún.
if not nombre_negocio or archivo_cargado is None:
    ...
    st.write("Sigue estos 5 pasos ilustrados para obtener el reporte oficial de tus publicaciones en Facebook e Instagram:")

    # Organizamos la ayuda en pestañas (Tabs) para mantener la interfaz limpia
    tab_proceso, tab_ayuda_redes = st.tabs(["🚀 Paso a Paso con Capturas", "📱 Filtro unificado de Facebook e Instagram"])

    with tab_proceso:
        # Fila 1: Pasos 1 y 2 (Imágenes más grandes para dar contexto de inicio)
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("##### 1. Accede a Meta Business Suite")
            st.caption("Desde el menú de tu perfil o página de Facebook, despliega las opciones de la derecha y haz clic en **Meta Business Suite**.")
            st.image("ImgRef/image_c584b6.jpg", caption="Paso 1: Acceso desde la interfaz principal", use_container_width=True)
            
        with col2:
            st.markdown("##### 2. Ve a la sección 'Contenido'")
            st.caption("Una vez dentro del panel de Meta Business, busca y selecciona la opción **Contenido** en el menú lateral izquierdo.")
            st.image("ImgRef/image_c58498.jpg", caption="Paso 2: Menú lateral > Contenido", use_container_width=True)
            
        st.write("---")
        
        # Fila 2: Pasos 3, 4 y 5 (Alineación compacta para la secuencia técnica)
        col3, col4, col5 = st.columns(3)
        
        with col3:
            st.markdown("##### 3. Haz clic en 'Exportar datos'")
            st.caption("En la esquina superior de la tabla de publicaciones, define el rango de fechas que deseas analizar (ej. últimos 90 días) y presiona el botón **Exportar datos**.")
            st.image("ImgRef/image_c5847b.jpg", caption="Paso 3: Ajustar fechas y exportar", use_container_width=True)
            
        with col4:
            st.markdown("##### 4. Configura y genera el reporte")
            st.caption("En la ventana emergente, verifica que la casilla **Publicación** esté marcada en el nivel de contenido y haz clic en el botón azul **Generar**.")
            st.image("ImgRef/image_c5845f.jpg", caption="Paso 4: Parámetros del reporte", use_container_width=True)
            
        with col5:
            st.markdown("##### 5. Descarga el archivo final")
            st.caption("Si la descarga no inicia sola, haz clic en la **flecha pequeña** junto al botón de exportar para ver tus 'Exportaciones recientes' y descargar tu `.csv` listo.")
            st.image("ImgRef/image_c583dd.jpg", caption="Paso 5: Historial de descargas listas", use_container_width=True)

    with tab_ayuda_redes:
        st.markdown("#### 💡 ¿Cómo asegurarte de incluir tanto Facebook como Instagram?")
        col_info_izq, col_info_der = st.columns([2, 1])
        
        with col_info_izq:
            st.write("""
            **¡Con un solo archivo es suficiente!** Meta reúne el contenido de ambas plataformas en el mismo lugar, pero antes de exportar asegúrate de lo siguiente:
            
            1. **Revisa los iconos debajo de tus títulos:** En la columna de contenidos (como se ve en la captura del Paso 3), comprueba si aparecen tanto el logo azul de **Facebook** como el logo rosa de **Instagram**.
            2. **Usa los filtros superiores:** Si notas que te falta una red, ve al filtro de **Tipo de publicación** o **Plataforma** en la parte superior central de la tabla y asegúrate de que ambas casillas estén marcadas antes de hacer la exportación.
            3. **Carga directa:** Una vez que tengas el archivo descargado en tu computadora, colócalo en el casillero de la izquierda de esta aplicación para iniciar el diagnóstico predictivo.
            """)
        
        with col_info_der:
            st.image("ImgRef/image_c5847b.jpg", caption="Verificación de canales en la grilla", use_container_width=True)

    st.write("---")
    st.info("💡 **Nota de seguridad y privacidad:** Este sistema procesa los archivos de forma volátil en la memoria de tu navegador local. Ningún dato de tu negocio, copys o métricas comerciales es almacenado ni enviado a servidores externos.")
    st.write("---")

# =========================================================================
# 4. SINCRONIZACIÓN CRONOLÓGICA CON EL MES REAL DE EJECUCIÓN (LÓGICA TEMPORAL)
# =========================================================================
# Detectamos automáticamente el mes real de ejecución de la máquina local
...
# Banco estacional parametrizado de hitos comerciales sin caracteres especiales problemáticos para fuentes vectoriales
banco_hitos_anuales = {
...
if archivo_cargado is not None:
    try:
        ...
        # Diccionario de sinonimias para mapear las cabeceras nativas de Meta en Español o Inglés
        dicc_sinonimos = {
        ...
        # Ejecutamos la unificación estructural de columnas de forma dinámica
        ...
        if all(col in df_raw.columns for col in columnas_requeridas):
            # SOLUCIÓN CRÍTICA: Copiamos el DataFrame completo para evitar advertencias de fragmentación de memoria
            df_fb = df_raw.copy()
            
            # Casteamos tipos de datos y limpiamos nulos numéricos de forma segura
            ...
            # Listas para almacenar las variables cronológicas derivadas
            horas_limpias = []
            dias_semana = []
            meses_publicacion = []
            dias_espanol = {0: 'Lunes', 1: 'Martes', 2: 'Miércoles', 3: 'Jueves', 4: 'Viernes', 5: 'Sábado', 6: 'Domingo'}
            
            # Parsing robusto de fechas y horas para absorber diferentes configuraciones regionales
            ...
            # Segmentación analítica por Trimestres del año (Cohortes de negocio)
            condiciones_q = [
            ...
            # Ordenamos categóricamente los días de la semana para los ejes de las gráficas
...
# =========================================================================
# 6. ENRUTADOR DE NAVEGACIÓN Y MENÚ CENTRAL SÍNCRONO
# =========================================================================
if not data_lista:
    ...
    # =========================================================================
    # 7. MOTOR DE PROCESAMIENTO DE LENGUAJE NATURAL (PLN SEMÁNTICO DE TEXTO)
    # =========================================================================
    # Une todos los copys de la marca, los limpia mediante Regex y extrae el top 4 de palabras clave recurrentes
    ...
    # =========================================================================
    # 8. MÓDULO DE MÉTRICAS ESTADÍSTICAS Y EXTREMOS HISTÓRICOS
    # =========================================================================
    ...
    # =========================================================================
    # 9. MODELO MATRICIAL DE REGRESIÓN LINEAL (MACHINE LEARNING)
    # =========================================================================
    # Entrenamos un estimador lineal usando One-Hot Encoding para los formatos de post detectados
    ...
    # Calculamos de forma paramétrica los índices de crecimiento comercial y márgenes de error de dispersión
    ...
    # Matriz Heurística unificada de Consultoría Avanzada
    recomendaciones_top10 = [
        ...
        f"7. Mineria de Palabras Clave de Exito: Inyectar en los primeros parrafos de tus copys los conceptos conceptuales dominantes: '{top_conceptos[0].upper()}' and '{top_conceptos[1].upper()}'.",
    ...
    # =========================================================================
    # SECCIÓN 3: PLANIFICADOR PROTOTIPO MENSUAL (TABLA VIVA SÍNCRONA)
    # =========================================================================
    ...
    # =========================================================================
    # SECCIÓN 4: TIMELINE, CRONOGRAMA Y DIAGNÓSTICO ESTRATÉGICO
    # =========================================================================
        ...
        fig_t1 = go.Figure(go.Scatter(x=df_dias['Dia_Semana'].astype(str), y=df_dias['Interacciones'], mode='lines+markers', line=dict(color='#85C1E9', width=4), marker=dict(size=8, color='#5DADE2')))
    ...
    # =========================================================================
    # SECCIÓN 5: REPORTE PDF (LIENZO MULTIPÁGINA COMPLETO Y CORREGIDO)
    # =========================================================================
        ...
        # Instanciamos una figura larga (22 pulgadas de alto) que contendrá de manera explícita dos sub-páginas independientes
        fig = plt.figure(figsize=(11, 22))
        
        # Paleta cromática corporativa libre de estridencias visuales
        lista_colores_base = ['#A9DFBF','#F9E79F','#F5B7B1','#AED6F1','#D2B4DE']
        colores_render = lista_colores_base[:max(1, len(df_agrupado))]
        
        # ---------------------------------------------------------------------
        # HOJA 1: ANALÍTICA AVANZADA Y AUDITORÍA DE FORMATOS (Cojinete superior de Y: 0.50 a 1.0)
        # ---------------------------------------------------------------------
        ax_h1 = fig.add_axes([0, 0.50, 1, 0.50])
        ax_h1.axis('off')
        
        # Construcción de fondos geométricos estructurados
        ...
        # Contenedor de Métricas Base (Lado Izquierdo)
        ...
        # Contenedor de Predicciones del Engine ML
        ...
        # Cuadrante de Gráficos de Formato Matplotlib
        ...
        # Cuadrante Inferior Cronológico de Hoja 1
        ...
        ax_h1.text(0.05, 0.019, "Universidad Casa Grande - Proyecto Integrador - Datos consolidados e indexados.", color='#1A237E', fontsize=8, fontweight='bold', zorder=3)
        
        # ---------------------------------------------------------------------
        # HOJA 2: MATRIZ DE PLANIFICACIÓN SEMANAL Y DIRECTRICES (Cojinete inferior de Y: 0.0 a 0.50)
        # ---------------------------------------------------------------------
        ...
        # Bloque Tabla Semanal Parametrizada Síncrona en PDF
        ...
        # Bloque Unificado Completo de Directrices Heurísticas de Control Operativo
        ...
        txt_b4 = (
            ...
            f"5. Amortiguar caídas algorítmicas los días {dia_valle.upper()} usando solo historias de interacción.\n\n"
            ...
            f"8. Mitigar desviaciones mediante pruebas A/B usando el margen predictivo controlado de +-{margen_error:.1f}%.\n\n"
            f"9. Forzar alcance orgánico inicial respondiendo comentarios durante los primeros 20 minutos de publicación.\n\n"
            f"10. Asegurar coherencia comunicacional adaptando los copys al hito estacional activo de {mes_actual_nombre.upper()}."
        )
        ax_h2.text(0.06, 0.49, txt_b4, color='#37474F', fontsize=8.5, linespacing=1.2, verticalalignment='top', zorder=3)
        
        # Cintillo inferior de Hoja 2 y compilación final de Bytes
        ...
        # Guardamos la figura en memoria y creamos el botón nativo de descarga
